...
def org_register(request):
    if request.method == "POST":
        ...
        if password1 == password2:
            ...
            subject = 'EPMS - OTP Verification'
            message = f'Hi {o_name}, thank you for registering in EPMS . Your One Time Password (OTP) for verfication is {otp}'
            email_from = settings.EMAIL_HOST_USER
...
def org_change_password(request):
    if request.method == 'POST':
        ...
        if org_details:
            subject = 'EPMS - Password Changed'
            message = f'Hi, your password was changed successfully! From EPMS'
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [o_email, ]
            send_mail(subject, message, email_from, recipient_list, fail_silently=True)
            ...
            subject = 'EPMS - Notifications'
            message = f'Hi, there was attempt to change your password! From EPMS'
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [o_email, ]
            send_mail(subject, message, email_from, recipient_list, fail_silently=True)
...
def org_forgot_password(request):
    if request.method == 'POST':
        ...
        if org_details:
            ...
            subject = 'EPMS - OTP Verification for Forgot Password'
...
def org_forgot_password_change_password(request):
    if request.method == 'POST':
        ...
        if(pwd1 == pwd2):
            ...
            if org_details:
                subject = 'EPMS - Password was Changed'
...
def report_org(request):
    if request.method == 'POST':
        ...
        recipient_list = ["narender.rk10@gmail.com",
                          ...
                          "2021.chinmay.vyapari@ves.ac.in"]
        send_mail(subject, message, email_from, recipient_list, fail_silently=True)
        send_mail(subject, "Your Problem has been recorded. From: EPMS", email_from, [cemail])
...
def add_emp(request):
    if request.method == 'POST':
        ...
        if empObj:
            subject = 'EPMS - Login Info'
            org_name = request.session['o_name']
            message = f'Name : {e_name}, \n Email : {e_email}, \n Password : {e_password} \n Organization : {org_name}, \n FROM - EPMS \n Developed by Jeremiah Muchazondida'
            email_from = settings.EMAIL_HOST_USER
            send_mail(subject, message, email_from, [e_email], fail_silently=True)
        ...
        if projAssignCheck:
            ...
            if projAssignCheckObj:
                ...
                if user_details and project_details:
                    subject = 'EPMS - Project was Assigned to you'
                    message = f'Hi, {s_name} You are asssigned to project {s_p_name} Check on EPMS !'
                    email_from = settings.EMAIL_HOST_USER
        ...
        if meetingObj:
            ...
            if user_details:
                for ud in user_details:
                    subject = 'EPMS - New Meeting'
                    s_name = ud['e_name']
                    s_email = ud['e_email']
                    message = f'Hi, {s_name} New Meeting for created for you! Details are Meeting Name :{m_name}, Meeting Description: {m_desc}, Date Time: {start_date} {start_time} Check on EPMS !'
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list, fail_silently=True)
            ...
            if meet_details:
                ...
                if user_details:
                    for ud in user_details:
                        ...
                        s_name = ud['e_name']
                        s_email = ud['e_email']
                        message = f'Hi, {s_name} New Meeting for created for you! Details are Meeting Name :{meet_name}, Meeting Description: {meet_desc}, Date Time: {start_date} {start_time} Check on EPMS !'
                        email_from = settings.EMAIL_HOST_USER
                        recipient_list = [s_email]
                        send_mail(subject, message, email_from, recipient_list, fail_silently=True)
...
def create_task(request):
    ...
    if request.method == 'POST':
        ...
        if taskObj:
            ...
            subject = 'EPMS - New Task Created for you'
            message = f'Hi {empDetails[0]["e_name"]} , Your organization as created a new task : {t_name} , description : {t_desc}, priority : {t_priority} and deadline for task is : {t_deadline_date}, Login in your account to get more information. From: EPMS. '
            email_from = settings.EMAIL_HOST_USER
...
def update_task(request, pk):
    try:
        ...
        if request.method == 'POST':
            ...
            if tasks:
                empDetails = Employee.objects.filter(id=e_id, o_id_id=o_id).values()
                subject = 'EPMS - Task Updated for you'
                message = f'Hi {empDetails[0]["e_name"]} , Your organization as updated a task : {t_name} , description : {t_desc}, priority : {t_priority} and deadline for task is : {t_deadline_date}, Login in your account to get more information. From: EPMS. '
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [empDetails[0]["e_email"], ]
                send_mail(subject, message, email_from, recipient_list, fail_silently=True)
...
#             print(e_id)
#             print(m_date)
#             m_date = datetime.datetime.strptime(m_date, '%Y-%m-%d').date()
#             a_status = 'LoggedIn_then_Out'

#             # Retrieve attendance logs for the employee and date
#             attendance_logs = AttendanceLogs.objects.filter(o_id_id=o_id, e_id_id=e_id, a_date=m_date, a_status=a_status).order_by('a_time')   
...
def user_view_attendance(request):
    try:
        ...
        if request.method == 'POST':
            ...
            # Retrieve attendance logs for the employee and date
            attendance_logs = AttendanceLogs.objects.filter(e_id_id=e_id, a_date=m_date, a_status=a_status).order_by('a_time')   
            print('attendance_logs:', attendance_logs)
        ...
        messages.error(request,"Data not found or some error was occurred!")
        return HttpResponseRedirect('/user-view-attendance')
...
#         logged_in_time = list(AttendanceLogs.objects.filter(e_id=e_id,a_date=m_date, a_status='Logged In').values_list('a_time'))[0][0]
#         logged_out_time = list(AttendanceLogs.objects.filter(e_id=e_id,a_date=m_date, a_status='Logged Out').values_list('a_time'))[0][0]
#         logged_in_time = datetime.datetime.fromtimestamp(int(logged_in_time)).strftime('%H:%M:%S')
#         logged_out_time = datetime.datetime.fromtimestamp(int(logged_out_time)).strftime('%H:%M:%S')
#         total_time_logged = datetime.datetime.strptime(logged_out_time,"%H:%M:%S") - datetime.datetime.strptime(logged_in_time,"%H:%M:%S")
#         context = {
...
def create_notice(request):
    ...
    if request.method == 'POST':
        ...
        if noticeObj:
            ...
            if user_details:
                for ud in user_details:
                    subject = 'EPMS - New Notice Published'
                    s_name = ud['e_name']
                    s_email = ud['e_email']
                    message = f'Hi, {s_name} Your Organization has published a new notice, Notice Title: {on_title} Check on EPMS !'
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list, fail_silently=True)
...
def get_work_productivity_details(o_id, e_id, m_date):
        ...
        print(emp_work_data_details_unclean)
...
def rank_productivity(request):
    ...
    fig = go.Figure(data=data)
    fig.update_layout(
        width=1300, # set the width of the plot to 800 pixels
        height=750, # set the height of the plot to 600 pixels
    )
    plot_div = plot(fig, output_type='div')
...
def logout(request):
    if request.method == 'POST':
        try:
            for key in list(request.session.keys()):
                del request.session[key]
                print("Logged Out Successifully")
